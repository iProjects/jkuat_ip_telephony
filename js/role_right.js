...
$(document).ready(function () {
    ...
    $('#btncreate_role_right_view').on('click', function(){
		clear_logs();
	...
	$('#create_role_right_modal').on('shown.bs.modal', function () {
		$('#cbo_create_role').focus();
		fetch_roles();
		fetch_rights();
	})  
	
    $('#btncreate_role_right').on('click', function(){
        create_role_right();
    });
	    
    $('#btnupdate_role_right').on('click', function(){
        update_role_right();
    });
    ...
    $('#txt_search_right_name').on('input', function(){
        search_roles_rights(1);
    });
    ...
    $('#btnclose_create_role_right_modal').on('click', function(){
        clear_logs();
    });
	   
    $('#btnclose_edit_role_right_modal').on('click', function(){
        clear_logs();
		search_roles_rights(1);		
    });
...
function resize_components() { 
	...
	$('#create_role_right_modal').css('margin-top', function() {
		return $('#div_navigation').height();
	});
 
	$('#edit_role_right_modal').css('margin-top', function() {
		return $('#div_navigation').height();
	});
...
function create_role_right(){
	...
	$.ajax({
		...
		$('#create_role_right_modal').modal('hide');
...
function get_delete_prompt(id){
	...
	$.ajax({
		...
		var delete_prompt = "Are you sure you wish to delete Right [ " + right_id + " ].";
...
function search_roles_rights(page){
	...
	var right_name = $("#txt_search_right_name").val(); 
	...
	$.ajax({
		...
		$('#role_right_container').css({'display' : 'block'});
	 
		$('#div_edit_role_right_container').css({'display' : 'none'});
...
function get_roles_rights_search_count(){
	...
	$.ajax({
		...
		type: "POST",
		data: { 
			"action": "get_roles_rights_search_count"
		},//data to be posted
	}).done(function(response){
		response = response.trim();
...
function go2Page(total_pages)
{
	...
	if(current_page > total_pages)
	{
		...
		search_roles_rights(current_page);
	}
}
...
function fetch_rights() {
	...
	$.ajax({
		...
		var rights_arr = JSON.parse(response);
		...
		for (var i = 0; i < rights_arr.length; i++) {
			var id = rights_arr[i].id;
			var right_name = rights_arr[i].right_name;
			console.log(id);
			console.log(right_name);
			select_options_arr.push('<option value="' + id + '">' + right_name + '</option>');
		}
 
		$('#cbo_create_right').html(select_options_arr);
		$('#cbo_edit_right').html(select_options_arr);
