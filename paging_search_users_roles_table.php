...
echo "<div id='div_pagination'>";
	...
	// first page button
	echo "<li>";
		echo "<a onClick='search_users_roles(1)' style='cursor: pointer !important;'>";
			echo "First";
		echo "</a>";
	...
	if($page>1){
		...
		$prev_page = $page - 1;
		echo "<li>";
			echo "<a onClick='search_users_roles({$prev_page})' style='cursor: pointer !important;'>";
				echo "Previous &laquo;";
			echo "</a>";
			/* echo "<a href='{$page_url}page={$prev_page}'>";
				echo "<span style='margin:0 .5em;'>&laquo;</span>";
	...
	// clickable page numbers
	 ...
	 $total_rows = $_SESSION['users_roles_count'];
	 //echo $total_rows;
	...
	for ($x=$initial_num; $x<$condition_limit_num; $x++) {
		...
		if (($x > 0) && ($x <= $total_pages)) {
			...
			// not current page
				...
				echo "<li>";
				echo " <a onClick='search_users_roles({$x})' style='cursor:pointer !important;'>{$x}</a> ";
					// echo " <a href='{$page_url}page={$x}' onClick='search_users_roles({$x})' style='cursor:hand;'>{$x}</a> ";
				echo "</li>";
			}
		}
	}
	...
	if($page<$total_pages){
		...
		echo "<li>";
			echo "<a onClick='search_users_roles({$next_page})' style='cursor: pointer !important;'>";
				echo "Next &raquo;";
			echo "</a>";
		...
		echo "<li>";
			echo "<a onClick='search_users_roles({$total_pages})' style='cursor: pointer !important;'>";
				echo "Last";
			echo "</a>";
			/* echo "<a href='{$page_url}page={$next_page}'>";
				echo "<span style='margin:0.5em;'>&raquo;</span>";
