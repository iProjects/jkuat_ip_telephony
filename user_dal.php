...
class user_dal
{
    ...
    public function delete_user($id)
    {
		try{
			...
			$response = "<div class='alert alert-success'>User with Email [ " . $email . " ] for [ " . $username . " ]  was successfully deleted.</div>";
	...
	public function search_users_v2($page, $records_to_display, $email, $username)
	{		
		// PAGINATION VARIABLES
		// page is the current page, if there's nothing set, default is page 1
		//$page = isset($_POST['page']) ? $_POST['page'] : 1;
		...
		*/
		...
		if(!empty($email) && !empty($username))
		{
			...
			$count_query = "SELECT * FROM telephoneadmin WHERE (email LIKE :email AND username LIKE :username) ORDER BY email, username ASC";
			...
			$count_stmt->bindParam(":username", $pattern, PDO::PARAM_STR);
		...
		else if(!empty($email) && empty($username))
		{
			...
			$count_query = "SELECT * FROM telephoneadmin WHERE email LIKE :email ORDER BY email ASC";
		...
		else if(empty($email) && !empty($username))
		{
			...
			$count_query = "SELECT * FROM telephoneadmin WHERE username LIKE :username ORDER BY username ASC";
		...
		{				
			...
			$count_query = "SELECT * FROM telephoneadmin ORDER BY id DESC";
